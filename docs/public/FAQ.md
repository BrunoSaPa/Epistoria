...
The current development generation imports version 8 packages into an empty notebook. Epistoria
validates the package first, then encrypts its records and original files for the target notebook.
Import does not merge with or replace existing data. Earlier development exports remain readable
archives and are not accepted by the current importer.
